...
\  /\  /| |_| | \ V / |  __/| |   | | | |
 \/  \/  \__, |  \_/   \___||_|   |_| |_|
         ...
         |___/                           
        ''' + Style.RESET_ALL)
        print_statement("Hint: 1:> Help, 2:> Quit")
        try:
            ...
            if choice == 1:
                try:
                    print('''
                           ...
                           Show run     | Task agent to compare running config with NVRAM.
                    ...
                    if help_command == 'Main': # Returns user to the main menu
                        ...
                        print("HQ Router: 192.168.92.137") #192.168.1.1
                        ...
                        if another_loop == "192.168.92.137":
                        ...
                        if rip_confirmation == "Yes":
                            ...
                            if yetanotherloop == "192.168.92.137": #192.168.1.1
                                print_statement("Selected 192.168.1.1")
                        ...
                        os.system("cat tmp.log")
                        print("\n")
                        running_config = str(input(
                            "(Wyvern):> Yes/No Confirmation, Would you like to run the command Show run for this device?  "))
                        ...
                        running_config_confirmation = str(input(
                            Fore.YELLOW + "(Wyvern):> Yes/No Confirmation, Are you absolutely sure? " + Style.RESET_ALL))
...
def paramiko_rip_config(M):  # Responsible for making the connection to the device.
    ...
    #ssh_shell.send("terminal length 0") #might not need this.
    ssh_shell.send("conf t\n")
    ssh_shell.send("router rip\n")
    ssh_shell.send("version 2\n")


    if M == "192.168.92.137": #HQ 192.168.1.1
    ...
    # shell_print_statement(output) # Invoke special print function.
    file = output.strip()
    f = open('tmp.log', 'w')
    ...
    if Query == 'Yes' or 'yes':  # Display RIP output
...
# def shell_print_statement(output):
#     file = output.strip()
#     f = open('tmp.log', 'w')
#     f.write(file)
#     f.write('\n')
